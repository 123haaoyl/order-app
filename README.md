...
如果已经配置了 Supabase，但只想本地演示完整流程，可以访问：
...
演示模式不会写入线上 Supabase。顾客端提交的订单会保存在当前浏览器本地存储里，后台演示页需要先输入任意邮箱和密码登录，再读取这些本地订单。
...
- 已配置 Supabase 时提交订单到后台；未配置或使用 `?demo=1` 时保存为本地演示订单。
...
- 未配置 Supabase 或使用 `?demo=1` 时进入本地演示后台，仍需要先通过登录页，可查看和流转本地订单。
...
如果之前已经运行过旧版 SQL，现在需要支持后台删除订单，请重新运行最新版 `supabase-schema.sql`，里面新增了订单删除权限和 RLS policy。
